...
- Bumped the `@master4n/temporal-transformer` dependency to `^2.0.6`, which
  ships a cleaned dev toolchain (no deprecated `glob`/`inflight`, js-yaml
  advisory resolved). Runtime behaviour is unchanged.
